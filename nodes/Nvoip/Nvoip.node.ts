	...
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
				...
				options: [
					...
					},
					{
						...
						description: 'Send an automated voice message with the ability to receive DTMF input',
						action: 'Send interactive voice broadcast',
					},
					{
						name: 'Send Voice Broadcast',
						value: 'sendVoiceBlast',
						description: 'Send an automated voice message delivered as audio file or TTS',
				...
				options: [
					{
						...
						value: 'sendWhatsapp',
						description: 'Send WhatsApp template messages using Nvoip for automated communication',
			...
			},
			{
				...
				description: 'Text or audio containing the options (ex: "Press 1 for yes, 2 for no")',
			},
			{
					...
					for (const tpl of inst.data) {
						...
						if (Array.isArray(tpl.components)) {
							for (const c of tpl.components) {
								...
								if (typeof c?.example?.header_text === 'string')
									texts.push(String(c.example.header_text));
		...
		for (let i = 0; i < items.length; i++) {
			try {
				...
				if (resource === 'sms' && operation === 'sendSms') {
					...
					function countTemplatePlaceholders(selectedTemplate: any): number {
						let total = 0;
						if (Array.isArray(selectedTemplate?.components)) {
							for (const c of selectedTemplate.components) {
								if (typeof c?.text === 'string') {
									total += countPlaceholders(c.text);
								}
								if (Array.isArray(c?.example?.body_text)) {
									for (const t of c.example.body_text) total += countPlaceholders(String(t));
								}
								if (typeof c?.example?.header_text === 'string') {
									total += countPlaceholders(String(c.example.header_text));
								}
								if (Array.isArray(c?.buttons)) {
									for (const b of c.buttons) {
										if (typeof b?.text === 'string') total += countPlaceholders(b.text);
										if (typeof b?.url === 'string') total += countPlaceholders(b.url);
									}
								}
							}
						}
					...
					const requiredCount = countTemplatePlaceholders(selectedTemplate);

					if (requiredCount > 0 && allValues.length < requiredCount) {
						throw new NodeOperationError(
							this.getNode(),
							`Template ${templateId} requer ${requiredCount} variável(eis), mas apenas ${allValues.length} foi(foram) informada(s)`,
							{ itemIndex: i },
						);
					}
					...
					if (imageUrl) {
						const hasMediaHeader = Array.isArray(selectedTemplate.components) &&
							selectedTemplate.components.some((c: any) => String(c.type).toUpperCase() === 'HEADER' && c.format && c.format !== 'TEXT');
						if (hasMediaHeader) {
